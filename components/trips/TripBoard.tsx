...
export function TripBoard() {
  ...
  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column" }}>
      <header style={{ flex: "0 0 auto", padding: mob ? "12px 16px" : "16px 24px", borderBottom: `1px solid ${T.line}`, display: "flex", alignItems: "center", gap: mob ? 12 : 18, background: T.panel, flexWrap: mob ? "nowrap" : "wrap" }}>
        ...
        {mob ? (
          <div style={{ textAlign: "right", flex: "0 0 auto" }}>
            <div style={{ fontFamily: T.mono, fontSize: 16, fontWeight: 700, color: T.price, lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{b.spentDisp}</div>
          ...
          <div style={{ textAlign: "right", flex: "0 0 auto" }}>
            <div style={{ fontFamily: T.mono, fontSize: 13, fontVariantNumeric: "tabular-nums" }}>
              <span style={{ color: T.text, fontWeight: 600 }}>{b.spentDisp}</span>
      ...
      <nav aria-label="Trip views" role="tablist" style={{ flex: "0 0 auto", display: "flex", padding: "0 24px", borderBottom: `1px solid ${T.line}`, background: T.panel }}>
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              aria-selected={on}
              onClick={() => setTab(t.k)}
              style={{ background: "none", border: "none", cursor: "pointer", padding: "12px 2px", marginRight: 24, fontSize: 13.5, fontWeight: on ? 700 : 600, color: on ? T.text : T.dim, borderBottom: `2px solid ${on ? T.accent : "transparent"}`, transition: "color .15s", fontFamily: "inherit" }}
            >
              {`${t.label} · ${t.cnt}`}
            </button>
          );
      ...
      <main id="tp-main" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
